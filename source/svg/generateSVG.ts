...
const generatedPSNSVG = async (psnData: PsnData, color: string) => {
    ...
    let profileImage = Buffer.from(await (await axios.get(psnData.picture, {responseType: 'arraybuffer'})).data).toString('base64')
    ...
    for(let gameData of psnData['games']) {
        let gameImage = Buffer.from(await (await axios.get(gameData.picture, {responseType: 'arraybuffer'})).data).toString('base64')
