...
const generatedPSNSVG = async (psnData: PsnData, color: string) => {
    ...
    let profileImage = (await sharp(profileImageBuffer).resize(100,100).toBuffer()).toString('base64')
    ...
    for(let gameData of psnData['games']) {
        ...
        let gameImage = (await sharp(gameImageBuffer).resize(50,50).toBuffer()).toString('base64')
